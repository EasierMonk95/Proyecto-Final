...
{
    game= new Game(false);      //Funcion para abrir el juego de modo un jugador
    game->show();                       //Posteriormente se cierra el login
    close();
}

void Login::on_J2_clicked()
{
    game = new Game(true);  //Funcion para abrir el juego de modo dos jugadores
    game->show();               //Al igual que la anterior se cierra el login
    close();
}
...
void Login::on_LoadGame_clicked()           //Funcion para cargar una partida jugada anteriormente
{
    close();
}
